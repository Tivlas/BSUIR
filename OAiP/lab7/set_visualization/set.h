...
class set {
	...
	using Tree = AVL_Tree<key_type, value_type, key_compare>;
...
public:
	...
	std::pair<iterator, bool> insert(const value_type& value) { return tree.insert({value,value}); }

	std::pair<iterator, bool> insert(value_type&& value) { return tree.insert({value,value}); }
