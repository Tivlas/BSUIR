...
class executer : public Visitor, public std::enable_shared_from_this<executer> {
   private:
    parser parser;
    V<Error> errors_;
    V<SP<Decl>> decls_;
   ...
   public:
    executer(const std::filesystem::path&);
    void execute();
};

executer::executer(const std::filesystem::path& path) : parser(path) {
    auto command = "go run " + path.string();
...
void executer::error(pos_t pos, std::string msg) { errors_.push_back({pos, msg}); }

void executer::openScope(pos_t pos) {
    depth_++;
...
void executer::declare(SP<Node> decl, SP<Expr> type, SP<Scope> scope, ObjKind kind,
                       V<SP<IdentExpr>> idents) {
...
void executer::shortVarDecl(SP<AssignStmt> decl) {
    int n = 0;
...
void executer::resolve(SP<IdentExpr> ident, bool collectUnresolved) {
    if (ident->Obj != nullptr) {
        throw std::runtime_error(ident->Pos().ToString() + ": identifier" + ident->Name +
                                 " already declared or resolved");
    }
...
void executer::walkExprs(V<SP<Expr>> list) {
    for (auto node : list) {
        Walk(shared_from_this(), node);
    }
}

void executer::walkLHS(V<SP<Expr>> list) {
    for (auto expr : list) {
        expr = unparen(expr);
...
void executer::walkStmts(V<SP<Stmt>> list) {
    for (auto stmt : list) {
        Walk(shared_from_this(), stmt);
    }
}

bool executer::checkConstChange(SP<Expr> expr) {
...
SP<Expr> executer::checkSelectorFieldAndReturnItsType(SP<SelectorExpr> n) {
...
SP<Visitor> executer::Visit(SP<Node> node) {
...
bool executer::checkDeclarationLeftType(SP<Expr> type) {
...
bool executer::checkExprType(SP<Expr> expr, SP<Expr> typeToBeEqualTo) {
    auto typ = validateExprAndReturnType(expr);
    if (typ == nullptr) return false;
    return *typ == *typeToBeEqualTo;
}

void executer::compareCallArgs(V<SP<Expr>> callArgs, SP<FieldList> realArgs) {
    size_t len = 0;
...
SP<IdentExpr> executer::getFuncName(SP<CallExpr> call) {
...
SP<Expr> executer::callExprType(SP<Expr> call) {
...
bool executer::isUnknown(SP<IdentExpr> ident) {
    for (auto sc = topScope_; sc != nullptr; sc = sc->Outer) {
        auto obj = sc->Lookup(ident->Name);
...
SP<Expr> executer::validateExprAndReturnType(SP<Expr> expr) {
...
void executer::walkFuncType(SP<FuncTypeExpr> typ) {
...
void executer::resolveList(SP<FieldList> list) {
    if (list == nullptr) {
        return;
    }
...
void executer::declareList(SP<FieldList> list, ObjKind kind) {
    if (list == nullptr) {
        return;
    }
    for (auto f : list->List) {
...
void executer::walkRecv(SP<FieldList> recv) {
...
void executer::walkFieldList(SP<FieldList> list, ObjKind kind) {
    if (list == nullptr) {
        return;
    }
    resolveList(list);
...
void executer::walkTParams(SP<FieldList> list) {
    declareList(list, Typ);
    resolveList(list);
}

void executer::walkBody(SP<BlockStmt> body) {
    if (body == nullptr) {
        return;
    }
    openLabelScope();
